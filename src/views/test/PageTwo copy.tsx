...
const PageTwo = ({ navigation }: NavProps) => {
  ...
  const [webSocketData, setWebSocketData] = useState<{ ft: number; lp: number; o: number, h:number, l: number, v:number }[]>([]);
  ...
  useEffect(() => {
    ...
    newSocket.onmessage = msg => {
      ...
      try {
        ...
        // Store ft and lp values in the state
        if (response.ft && response.lp && response.o && response.h && response.l && response.v) {
